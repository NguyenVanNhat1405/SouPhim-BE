...
const MoviePopu = require('../Models/moviePopu.model'); // API key cho TMDb
...
async function getMoviePopu(req, res) {
    ...
    try {
        ...
        if (!movie) {
            ...
            await saveMovie(movieResponse, trailer, MoviePopu);

            await saveMovieToAllCollections(movieResponse, trailer, MoviePopu);
            movie = await MoviePopu.findOne({ id: movieId }); // Lấy lại phim đã lưu
        }
...
async function getAllMovies(req, res) {
    try {
        const movies = await MoviePopu.find(); // Lấy tất cả phim từ MongoDB
        res.json(movies);
    } catch (error) {
